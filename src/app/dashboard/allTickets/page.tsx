...
export default function AllTicketsPage() {
  ...
  const loadMoreTickets = useCallback(async (pageToLoad: number) => {
    if (isLoading || (pageToLoad > 1 && !hasMore)) { // Check hasMore only for subsequent pages
      console.log(`AllTicketsPage: loadMoreTickets - returning early. PageToLoad: ${pageToLoad}, isLoading: ${isLoading}, hasMore: ${hasMore}`);
      return;
    }

    console.log(`AllTicketsPage: loadMoreTickets called. Fetching page: ${pageToLoad}`);
    setIsLoading(true);
    try {
      ...
      setTickets((prevTickets) => {
        ...
        const newTickets = pageToLoad === 1 ? data.tickets : [...prevTickets, ...data.tickets];
        console.log(`AllTicketsPage: setTickets called. Previous length: ${prevTickets.length}, New items: ${data.tickets.length}, Resulting length: ${newTickets.length}`);
        return newTickets;
      });
      ...
      console.log(
        `AllTicketsPage: Load successful for page ${pageToLoad}. Next Page will be: ${newHasMoreState ? pageToLoad + 1 : pageToLoad}, HasMore: ${newHasMoreState}, TotalPages: ${data.totalPages}`
      );
    } catch (error) {
      console.error("AllTicketsPage: Failed to load tickets for page " + pageToLoad + ":", error);
      setHasMore(false); 
    }
    setIsLoading(false);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoading, hasMore]); // Removed currentPage, pass it as an argument

  // Effect for initial load
  useEffect(() => {
    if (!initialLoadEffectRan.current) {
        console.log("AllTicketsPage: Initial load effect. Resetting and fetching page 1.");
        setTickets([]);      
        setCurrentPage(1);   
        setHasMore(true);    
        setTotalTickets(0);  
  ...
  useEffect(() => {
    console.log(`AllTicketsPage: inView effect. InView: ${inView}, HasMore: ${hasMore}, IsLoading: ${isLoading}, CurrentPage: ${currentPage}`);
  ...
  useEffect(() => {
    console.log("AllTicketsPage: 'tickets' state has changed. New length:", tickets.length);
    if (tickets.length > 0) {
    ...
    <div className="container mx-auto p-4 py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">
          All Tickets ({totalTickets > 0 ? totalTickets : (isLoading && tickets.length === 0 ? "Loading..." : tickets.length)})
        </h1>
        <Button asChild variant="outline">
          <Link href="/dashboard">&larr; Back to Dashboard Summary</Link>
        </Button>
      </div>
      ...
      {!isLoading && hasMore && (
        <div ref={ref} className="h-10 py-4 text-center text-sm text-muted-foreground">
          {/* Scroll to load more... */}
        </div>
      )}
