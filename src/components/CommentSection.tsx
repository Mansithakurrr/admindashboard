...
export const CommentSection = ({ ticketId, onCommentAdded }: CommentSectionProps) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      const updated = data.map((comment: any) => ({
        ...comment,
        formattedTimestamp: new Date(comment.createdAt).toLocaleString("en-IN", {
          dateStyle: "medium",
          timeStyle: "short",
        }),
      }));
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/comments", {
      ...
      body: JSON.stringify({ text: newComment, author: "You", ticketId : ticketId}),
    });
    ...
    const updatedComment = {
      ...savedComment,
      formattedTimestamp: new Date(savedComment.createdAt).toLocaleString("en-IN", {
        dateStyle: "medium",
        timeStyle: "short",
      }),
    };
